...
#' @aliases cell.names<-,PropCellTypes-method
#'
#' @param value Matrix containing names of the mixed spots to be
...
#' @aliases test.metrics<-,DeconvDLModel-method
#'
#' @param value List with the resulting metrics after prediction on test data
#'   with the trained Deep Neural Network model.
#'
#' @export test.metrics<-
#'   
setGeneric(
...
#'
#' @param object \code{\linkS4class{SpatialDDLS}} object.
#' @param index.st Index of the ST data. It can be either an index or a name if
#'   a named list is provided. If \code{NULL} (by default), all data contained
...
#' @aliases spatial.experiments<-,SpatialDDLS-method
#'
#' @param value List in which every element must be a
...
#'
#' @param object \code{\linkS4class{SpatialDDLS}} object.
#' @param type.data Type of data to return. It can be \code{'train'},
...
#' @aliases prob.cell.types<-,SpatialDDLS-method
#'
#' @param value List with two elements corresponding to train and test, each one
#'   with a \code{\linkS4class{PropCellTypes}} object.
#'
#' @export prob.cell.types<-
#'   
setGeneric(
...
#' @aliases mixed.spot.profiles<-,SpatialDDLS-method
#'
#' @param value List with two elements, train and test, each one being a
#'   \code{\linkS4class{SummarizedExperiment}} object containing simulated mixed
...
#'
#' @param object \code{\linkS4class{SpatialDDLS}} object.
#' @param index.st Name or index of the results (same as for the
#'   \code{spatial.experiments} slot). If \code{NULL} (by default), all results
#'   are returned.
#'
#' @export deconv.spots
#'   
...
#' @aliases deconv.spots<-,SpatialDDLS-method
#'
#' @param value List with deconvolution results of the data stored in the
...
#' objects (e.g. RData or RDS files). By saving the structure as a JSON-like
...
#' state of optimizer is not saved, only the architecture and weights. It is
