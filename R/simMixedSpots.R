...
#' \code{\linkS4class{PropCellTypes}} object will contain all the information needed
#' to simulate new mixed transcriptional profiles. Note this function does not simulate the mixed profiles,
#' this task is performed by the \code{\link{simMixedProfiles}} or
...
#' 2/3 of the data for training and 1/3 for testing. The default setting for this 
#' ratio can be changed using the \code{train.freq.cells} parameter. Next, a 
#' total of \code{num.sim.spots} mixed proportions are simulated using a 
...
#'   simulated mixed sample. In addition to the Dirichlet-based proportions, 
#'   pure spots (containing only one cell type) and spots containing a specified 
#'   number of different cell types (determined by the \code{min.zero.prop} patameter) are 
#'   also generated in order to cover situations with only a few cell types present. 
#'   The proportion of simulated spots generated by each method can be controlled 
#'   using the \code{proportion.method} parameter. To visualize the distribution 
#'   of cell type proportions generated by each method, the 
...
#'   cells metadata.
#' @param cell.type.column Name or column number corresponding to cell types in cells metadata.
...
#' @param n.cells Specifies the number of cells to be randomly selected 
#' and combined to generate the simulated mixed profiles. By default, it is set 
#' to 100. It controls the level of noise present in the simulated data, as it 
#' determines how many single-cell profiles will be combined to produce each spot. 
...
#' @param proportion.method Vector with three elements that controls the proportion
#'   of simulated proportions generated by each method: random sampling of a Dirichlet distribution,
#'   "pure" spots (1 cell type), and spots with a specified number of different 
#'   cell types (determined by \code{min.zero.prop}), respectively.
#'    By default, 60% of the simulated proportions will follow a
#'   Dirichlet distribution, 20% will consist of only one cell type, and 20% will have 
#'   a specified number of different cell types. 
#' @param prob.sparity It only affects the proportions generated by the first method (Dirichlet distribution). 
#' It determines the probability of having missing cell types in each simulated 
#' spot, as opposed to a mixture of all cell types. A higher value for this
#'  parameter will result in more sparse simulated samples.
#' @param min.zero.prop This parameter controls the minimum number of cell 
#' types that will be absent in each simulated spot. If \code{NULL} (by default), this value will be half 
#' of the total number of different cell types, but increasing it will 
#' result in more spots composed of fewer cell types. This helps to create more 
#' sparse proportions and cover a wider range of situations during model training.
...
#' @export
#'
#' @seealso \code{\link{simMixedProfiles}}
...
#' composition matrices generated by the \code{\link{genMixedCellProp}} function.
#'
...
#'  known cell composition matrix. The number of
#' simulated spots and cells used to simulate each spot are determined by the
#' \code{\link{genMixedCellProp}} function. This step can
#' be avoided by using the \code{on.the.fly} argument in the
#' \code{\link{trainDeconvModel}} function.
#'
...
#' could be useful during some computationally heavy steps such as
...
#' subsequent transformations of the data will be done in blocks. 
#' Note that if you use the \code{file.backend} argument with
#' \code{block.processing = FALSE}, all mixed profiles will be simulated in
#' one step and, thus, loaded into RAM. Then, the matrix will be written to
...
#' \code{block.processing = TRUE}. We recommend
#' this option accordingly to the computational resources available and the number
...
#' @param mixing.function Function used to build mixed transcriptional profiles. It may be:
...
#'   RAM. Note that operations on this matrix will be performed in blocks
...
#'   This functionality is suitable for cases where it is not possible to load all
...
#'   \code{mixed.profiles} slot containing a list with one or two entries
#'   (depending on selected \code{type.data} argument): \code{'train'} and
#'   \code{'test'}. Each entry consists of a
#'   \code{\linkS4class{SummarizedExperiment}} object with the
