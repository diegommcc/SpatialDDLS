...
#' @param num.sim.spots  number of mixed spot profiles to be simulated. It is
#'   recommended to adjust this number according to the number of available
#'   single-cell profiles.
#' @param n.cells Number of cells that to be aggregated for each mixed spot (100
#'   by default).
#' @param train.freq.cells Proportion of cells used to simulate training mixed
#'   spot transcriptional profiles (2/3 by default).
#' @param train.freq.spots Proportion of mixed spot transcriptional profiles
#'   used for training (2/3 by default).
#' @param proportion.method Vector of two elements which controls the proportion
...
#' @export
#'
#' @seealso \code{\link{simMixedSpotProfiles}}
...
genMixSpotProp <- function(
  ...
  if (length(proportion.method) != 3) {
    stop("`proportion.method` must be a vector of three elements. See ?genMixSpotProp")
  ...
  if (is.null(min.zero.prop)) {
    ...
    if (min.zero.prop > n.cell.types - 2 | min.zero.prop < 2) {
      warning(
        "min.zero.prop cannot be greater than the number of cell types - 2. Seeting min.zero.prop = n.cell.types - 2",
        call. = FALSE, immediate. = TRUE
      )
...
# TODO: this function must be revised, is the one which generates a zero-inflated dirichlet dist
# generate index.ex variable for each sample using prob.sparsity
...
## same as before but without any prob and introducing the number of zeroes manually
...
#' @param mixing.function Function used to build pseudo-bulk samples. It may be:
...
#'   calculated.}
#' @param file.backend Valid file path to store simulated mixed spot expression
...
#' @export
#'
#' @seealso \code{\link{genMixSpotProp}} \code{\linkS4class{PropCellTypes}}
#'   \code{\link{trainDeconvModel}}
#'
#' @examples
#' set.seed(123) # reproducibility
...
simMixedSpotProfiles <- function(
  ...
  if (type.data == "both") {
    ...
    if (!is.null(mixed.spot.profiles(object)) && 
        type.data %in% names(mixed.spot.profiles(object))) {
      warning(
        paste(
          type.data, "data in 'mixed.spot.profiles' slot will be overwritten", 
          "\n\n"
        ),
        call. = FALSE, immediate. = TRUE
      )
    ...
    if (!is.null(mixed.spot.profiles(object))) {
      ...
      mixed.spot.profiles(object) <- c(
        mixed.spot.profiles(object), type.data = bulk.counts
      )
    } else {
