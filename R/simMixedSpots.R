...
genMixSpotProp <- function(
  ...
  ## here, the proportion between pure and dirichlet-derived proportions is decided
  proportions.train <- c(0.8, 0.2)
  nums.train <- .setHundredLimit(
    ceiling(total.train * proportions.train),
    limit = total.train
  )
  nums.test <- .setHundredLimit(
    ceiling(total.test * proportions.train),
    limit = total.test
  )
  if (verbose) {
