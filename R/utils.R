...
#' access to the cell composition matrix of simulated mixed transcriptional profiles.
#'
...
#'
#' Show distribution plots of the cell proportions generated by
#' the \code{\link{genMixedCellProp}} function. 
#' 
#' These frequencies will determine the
#' proportion of different cell types used during the simulation of the mixed 
#' transcriptional profiles. Proportions generated by each method (see \code{?\link{genMixedCellProp}}) 
#' can be visualized in three ways: box plots, violin plots, and
...
#' a trained DNN model. 
#' 
#' Since \pkg{keras} models cannot be saved natively as R 
#' objects, this function saves the structure of the model as a JSON-like 
#' character object and its weights as a list. This allows for the retrieval of 
#' the model and making predictions. It is important to note that the state of 
#' the optimizer is not saved, only the model's architecture and weights. To save 
#' the entire model, please see the \code{\link{saveTrainedModelAsH5}} and
...
#'   \code{trained.data} slot containing a \code{\linkS4class{DeconvDLModel}} object with 
#' a trained DNN model.
#'
...
#' \code{\linkS4class{DeconvDLModel}} object, only the trained \pkg{keras} model.
...
#'
#' @return No return value, saves a \pkg{keras} DNN trained model as HDF5 file on
#'   disk.
#'
#' @export
#'
...
#'   \code{\link{DeconvDLModel}} object will be formed, but will not
...
#'   metrics available in the \code{\linkS4class{DeconvDLModel}} object
#'   will be plotted.
#'
...
#'
#' This function facilitates the installation of the required Python dependencies 
#' for the SpatialDDLS package. It requires a Python interpreter with the 
#' TensorFlow Python library and its dependencies. It utilizes the reticulate 
#' package and the installer of the tensorflow R package to perform the 
#' installation. Conda environments will be used with the new 
#' environment being named SpatialDDLS-env. This function is intended to 
#' simplify the installation process for SpatialDDLS by automatically installing 
#' Miniconda and creating a new environment named SpatialDDLS-env. For users 
#' who wish to use a different Python or conda environment, see the 
#' tensorflow::use_condaenv function and the package vignettes for more information.
#'
...
#' @param install.conda Boolean indicating if installing miniconda automatically by
